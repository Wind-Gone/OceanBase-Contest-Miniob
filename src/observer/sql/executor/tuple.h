...
public:
  ...
  void add(int value, const std::string table_name ,bool is_null = false);
  ...
  void set(int index, std::shared_ptr<TupleValue> tuple_value) {
    values_[index] = tuple_value;
  }

  TupleValue &get_mutable(int index) {
    return *values_[index];
  }
...
public:
  ...
  void print(std::ostream &os, bool with_title) const;
public:
...
public:
  ...
  void print(std::ostream &os ,  bool with_title = false) const;

  template<typename V>
  void add_val(Tuple &tuple, int index) {
    sum_result += dynamic_cast<V &>(tuple.get_mutable(index)).get_value();
  }
  float get_avg() {
    return (float) (sum_result / this->size());
  }  
