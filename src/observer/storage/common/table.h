...
public:
  ...
  RC create_index(Trx *trx, const char *index_name, char *const *attribute_name, const int attribute_num, const bool unique);
...
private:
  ...
  // 查看对应列是否已经存在重复元素
  RC check_repeat(const FieldMeta *field_meta);
  // 查看新的值会不会和已有的值重复
  RC check_unique(const FieldMeta * field_meta, const Value * value);
