...
RC FieldMeta::init(const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible, bool unique, bool nullable) {
...
void FieldMeta::set_unique(bool new_unique) {
  unique_ = new_unique;
}
...
RC FieldMeta::from_json(const Json::Value &json_value, FieldMeta &field) {
  ...
  if (!unique_value.isBool()) {
    LOG_ERROR("Unique field is not a bool value. json value=%s", unique_value.toStyledString().c_str());
    return RC::GENERIC_ERROR;
  }
  if (!nullable_value.isBool()) {
    LOG_ERROR("Unique field is not a bool value. json value=%s", nullable_value.toStyledString().c_str());
    return RC::GENERIC_ERROR;
  }
  ...
  return field.init(name, type, offset, len, visible, unique, nullable);
}
