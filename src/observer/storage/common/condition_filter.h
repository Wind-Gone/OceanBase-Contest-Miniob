...
public:
  ...
  RC init(const ConDesc &left, const ConDesc &right, AttrType attr_type_left, AttrType attr_type_right, CompOp comp_op);
  RC init(Table &table, const Condition &condition);
  static CompOp swapOp(CompOp op);
...
public:
  ...
  AttrType get_attr_type_left(){
      return this -> attr_type_left_;
  }

  AttrType get_attr_type_right(){
      return this -> attr_type_right_;
  }
