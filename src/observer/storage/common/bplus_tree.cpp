...
RC BplusTreeHandler::get_entry(const char *pkey,RID *rid) {
  ...
  for(i=0;i<leaf->key_num;i++){
    if(CmpKey(file_header_.attr_type, file_header_.attr_length,key,leaf->keys+(i*file_header_.key_length))==0){ // TODO
...
RC BplusTreeHandler::find_first_index_satisfied(CompOp compop, const char *key, PageNum *page_num, int *rididx) {
  ...
  while(next > 0){
    ...
    for(i = 0; i < node->key_num; i++){
      ...
      if (compop == IS_NULL || compop == IS_NOT_NULL){
...
RC BplusTreeScanner::next_entry(RID *rid) {
  ...
  while(rc == RC::RECORD_NO_MORE_IDX_IN_MEM){
    rc = find_idx_pages();
    if(rc != SUCCESS){
      return rc;
    }
...
bool BplusTreeScanner::satisfy_condition(const char *pkey) {
  ...
  int attr_length = index_handler_.file_header_.attr_length - sizeof(bool);
  if (comp_op_ == IS_NULL || comp_op_ == IS_NOT_NULL) {
      return *(bool *)(pkey + attr_length) ? comp_op_ == IS_NULL : comp_op_ == IS_NOT_NULL;
    }
  if (*(bool *)(pkey + attr_length) || *(bool *)(value_ + attr_length)) {
    return false;
  }
