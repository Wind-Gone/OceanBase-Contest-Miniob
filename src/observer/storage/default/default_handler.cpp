...
RC DefaultHandler::drop_table(const char *dbname, const char *relation_name) {
  ...
  return db->drop_table(db,relation_name);
}

RC DefaultHandler::create_index(Trx *trx, const char *dbname, const char *relation_name, const char *index_name, char * const*attribute_name,const int attribute_num, const bool unique) {
  Table *table = find_table(dbname, relation_name);
  if (nullptr == table) {
    return RC::SCHEMA_TABLE_NOT_EXIST;
  }
  return table->create_index(trx, index_name, attribute_name, attribute_num, unique);
}
...
RC DefaultHandler::insert_record(Trx *trx, const char *dbname, const char *relation_name, int tuple_num, const InsertTuple *tuples) {
  Table *table = find_table(dbname, relation_name);
  if (nullptr == table) {
    return RC::SCHEMA_TABLE_NOT_EXIST;
  }
