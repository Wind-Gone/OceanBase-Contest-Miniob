...
void DefaultStorageStage::handle_event(StageEvent *event) {
  ...
  case SCF_INSERT: { // insert into
      ...
      rc = handler_->insert_record(current_trx, current_db, table_name, inserts.tuple_num, inserts.tuples);
  ...
  case SCF_CREATE_TABLE: { // create table
    ...
    break;
  case SCF_DROP_TABLE: { // create table
  ...
  case SCF_DROP_INDEX:{
      ...
      rc = handler_->drop_index(current_trx, current_db, drop_index.table_name, drop_index.index_name);
  }
    break;
  case SCF_CREATE_INDEX: {
      ...
      rc = handler_->create_index(current_trx, current_db, create_index.relation_name, 
                                  create_index.index_name, create_index.attribute_name, create_index.attribute_num, (create_index.unique & true)/* 类型转换 */ );
      snprintf(response, sizeof(response), "%s\n", rc == RC::SUCCESS ? "SUCCESS" : "FAILURE");
    }
    break;
