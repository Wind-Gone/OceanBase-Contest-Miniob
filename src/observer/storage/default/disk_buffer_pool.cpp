...
Frame* BPManager::alloc() {
  if (simple_alloc_cnt < size) {
    return frame + (simple_alloc_cnt++);
  }
  ...
  else if (!evict_list->empty()) { // needs evicition
    return evict();
  }
  return nullptr;
}
Frame* BPManager::get(int file_desc, PageNum page_num) {
  ull key = get_key(file_desc, page_num);
  Frame* ret = evict_list->get(key);
  if (ret != nullptr) return ret;
  auto iter = pinned_map.find(key);
  if (iter == pinned_map.end()) {
    return nullptr;
  } else {
    return iter->second;
  }
}
// add into evict list and remove from pinned_map, if pin count is zero
void BPManager::release_maybe(int file_desc, PageNum page_num, Frame* frame) {
  if (frame->pin_count <= 0) {
    ull key = get_key(file_desc, page_num);
    evict_list->put(key, frame);
    pinned_map.erase(key);
  }
}
// add into free list, and remove from evict list/pinned map
void BPManager::free(int file_desc, PageNum page_num, Frame* frame) {
  if (frame) {
    ull key = get_key(file_desc, page_num);
...
// add into pinned map and remove from evict list
void BPManager::add_pinned(int file_desc, PageNum page_num, Frame* frame) {
  ull key = get_key(file_desc, page_num);
  pinned_map[key] = frame;
  evict_list->free(key);
}
...
RC DiskBufferPool::open_file(const char *file_name, int *file_id)
{
  ...
  Frame* from_bp = nullptr; // possibly hdr is already loaded into memory
  if ((from_bp = bp_manager_.get(fd, 0))) {
    file_handle->hdr_frame = from_bp;
  } else if ((tmp = allocate_block(&file_handle->hdr_frame)) != RC::SUCCESS) {
  ...
  if (from_bp == nullptr && (tmp = load_page(0, file_handle, file_handle->hdr_frame)) != RC::SUCCESS) {
  ...
  *file_id = i - 1;
  bp_manager_.add_pinned(file_handle->file_desc, 0, file_handle->hdr_frame);
  LOG_INFO("Successfully open %s. file_id=%d, hdr_frame=%p", file_name, *file_id, file_handle->hdr_frame);
  return RC::SUCCESS;
}
// close file must free all files.
RC DiskBufferPool::close_file(int file_id)
{
  ...
  bp_manager_.free(file_handle->file_desc, file_handle->hdr_frame->page.page_num, file_handle->hdr_frame);
...
RC DiskBufferPool::get_this_page(int file_id, PageNum page_num, BPPageHandle *page_handle)
{
  ...
  Frame* frame;
  if ((frame = bp_manager_.get(file_handle->file_desc, page_num))) {
      frame->pin_count++;
      frame->acc_time = current_time();
      page_handle->open = true;
      page_handle->frame = frame;
      bp_manager_.add_pinned(file_handle->file_desc, page_num, frame);
      return RC::SUCCESS;
  }
...
RC DiskBufferPool::allocate_page(int file_id, BPPageHandle *page_handle)
{
  ...
  if ((tmp = flush_block(page_handle->frame)) != RC::SUCCESS) {
    ...
    page_handle->frame->pin_count = 0;
    bp_manager_.release_maybe(file_handle->file_desc, page_handle->frame->page.page_num, page_handle->frame);
    return tmp;
  }
  bp_manager_.add_pinned(file_handle->file_desc, page_handle->frame->page.page_num, page_handle->frame);
...
RC DiskBufferPool::unpin_page(BPPageHandle *page_handle)
{
  ...
  bp_manager_.release_maybe(page_handle->frame->file_desc, page_handle->frame->page.page_num, page_handle->frame);
  return RC::SUCCESS;
}
...
RC DiskBufferPool::dispose_page(int file_id, PageNum page_num)
{
  ...
  auto ret = bp_manager_.get(file_handle->file_desc, page_num);
  if (ret && ret->pin_count != 0) {
  ...
  bp_manager_.free(file_handle->file_desc, page_num, ret);
  return RC::SUCCESS;
}
...
RC DiskBufferPool::force_page(BPFileHandle *file_handle, PageNum page_num)
{
  ...
  if (ret) {
    if (ret->pin_count != 0) {
      ...
      if (ret->dirty) {
        RC rc = RC::SUCCESS;
        if ((rc = flush_block(ret)) != RC::SUCCESS) {
          LOG_ERROR("Failed to flush page:%s:%d.", file_handle->file_name, page_num);
          return rc;
        }
      }
      bp_manager_.release_maybe(file_handle->file_desc, page_num, ret);
    }
  }
...
RC DiskBufferPool::force_all_pages(BPFileHandle *file_handle)
{
  for (int i = 0; i < BP_BUFFER_SIZE; i++) {
    if (bp_manager_.frame[i].dirty) {
      ...
      if (file_handle->hdr_frame != (bp_manager_.frame + i)) {
        if (file_handle->hdr_frame->pin_count == 0) { // close file
          bp_manager_.free(bp_manager_.frame[i].file_desc, bp_manager_.frame[i].page.page_num, bp_manager_.frame + i);
        } else {
          bp_manager_.release_maybe(bp_manager_.frame[i].file_desc, bp_manager_.frame[i].page.page_num, bp_manager_.frame + i);
        }
      }
    }
  }
...
RC DiskBufferPool::allocate_block(Frame **buffer)
{
  ...
  Frame* ret = bp_manager_.alloc();
  if (ret) {
    if (ret->dirty) {
      RC rc = flush_block(ret);
      if (rc != RC::SUCCESS) {
        LOG_ERROR("Failed to flush block of %d for %d.", ret - bp_manager_.frame, ret->file_desc);
        return rc;
      }
    }
    *buffer = ret;
    LOG_DEBUG("Allocate block frame=%p", *ret);
    return RC::SUCCESS;
  } else {
...
RC DiskBufferPool::dispose_block(Frame *buf)
{
  ...
  buf->dirty = false;
  bp_manager_.free(buf->file_desc, buf->page.page_num, buf);
  LOG_DEBUG("dispost block frame =%p", buf);
  return RC::SUCCESS;
}

RC DiskBufferPool::is_file_open(int file_id){
  return open_list_[file_id] ? RC::SUCCESS : RC::BUFFERPOOL_ILLEGAL_FILE_ID;
}
